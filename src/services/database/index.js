...
export const insertNewChampionship = ({ name, pictureURI }) =>
...
export const deleteChampionship = ({ championshipId }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
          const championship = realm.objectForPrimaryKey(
            CHAMPIONSHIP_SCHEMA,
            championshipId,
          );
...
export const getChampionship = ({ championshipId }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
          ...
          Array.from(championship.teams).map((team, index) => {
            retChampionship.teams[index].players = proxyToArray(
              Array.from(team.players),
            );
          });
...
export const getAllChampionships = () =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        const allChampionships = proxyToArray(
          Array.from(realm.objects(CHAMPIONSHIP_SCHEMA)),
        );
...
export const insertNewTeam = ({ championshipId, name, pictureURI }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
...
export const insertNewGame = ({ championshipId, homeId, awayId, date }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
  ...
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
          if (!id) {
            reject(new Error('You need to inform the Game Id to update'));
          }
...
export const getPlays = ({ gameId, teamId, filter = '' }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
          const game = realm.objectForPrimaryKey(GAME_SCHEMA, gameId);
          const plays =
            game.homeId === teamId ? game.homePlays : game.awayPlays;
...
export const insertNewPlayer = ({ teamId, name, pictureURI, number }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
...
export const updatePlayerPos = ({ id, xPos, yPos }) =>
  new Promise((resolve, reject) => {
    Realm.open(database)
      .then(realm => {
        realm.write(() => {
          if (!id) {
            reject(new Error('You need to inform the Player Id to update'));
          }
